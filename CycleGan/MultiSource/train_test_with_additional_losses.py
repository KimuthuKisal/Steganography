...
def train_function_double(experiment_name:str, discriminator_T:Discriminator, discriminator_S:Discriminator, generator_S:Generator, generator_T:Generator, dataloader:DataLoader, discriminator_optimizer, generator_optimizer, L1_loss, MSE_loss, discriminator_scaler, generator_scaler, epoch:int, refine_technique:str = "4nn"):
    ...
    intermediate_loss_array = []
    source_refine_loss_array = []
...
# =================================================================================== #
    ...
    intermediate_loss_value = sum(intermediate_loss_array) / len(intermediate_loss_array)
    source_refine_loss_value = sum(source_refine_loss_array) / len(source_refine_loss_array)
    ...
    intermediate_loss = f"{experiment_name}_SavedLossFiles/intermediate_loss.txt"
    with open(intermediate_loss, "a") as intermediate_loss_file:
        intermediate_loss_file.write(str(intermediate_loss_value) + "\n")

    source_refine_loss = f"{experiment_name}_SavedLossFiles/source_refine_loss.txt"
